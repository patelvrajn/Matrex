...
constexpr Matrex_FP_Int CORR_HIST_DIAGONALS_WEIGHT =
    Matrex_FP_Int::from_double(0.15);
constexpr Matrex_FP_Int CORR_HIST_ORTHOGONALS_WEIGHT =
    Matrex_FP_Int::from_double(0.15);
constexpr Matrex_FP_Int CORR_HIST_KNIGHTS_WEIGHT =
    Matrex_FP_Int::from_double(0.15);
constexpr Matrex_FP_Int CORR_HIST_MATERIALS_WEIGHT =
    Matrex_FP_Int::from_double(0.2);
                                             ...
                                             Score static_evaluation)
{
    ...
    Score& pawns_entry =
        m_tables[side_to_move].pawns_table[indices.pawns_index];
    Score& diagonals_entry =
        m_tables[side_to_move].diagonals_table[indices.diagonals_index];
    Score& orthogonals_entry =
        m_tables[side_to_move].orthogonals_table[indices.orthogonals_index];
    Score& knights_entry =
        m_tables[side_to_move].knights_table[indices.knights_index];
...
template <std::size_t size>
Score Correction_History_Tables<size>::get_correction(
    const Chess_Board& position) const
{
    const PIECE_COLOR side_to_move = position.get_side_to_move();
    ...
    const auto pawns_entry = m_tables[side_to_move]
                                 ...
                                 .to_fixed_point();
    const auto diagonals_entry = m_tables[side_to_move]
                                     .diagonals_table[indices.diagonals_index]
                                     .to_fixed_point();
    const auto orthogonals_entry =
        m_tables[side_to_move]
            .orthogonals_table[indices.orthogonals_index]
            .to_fixed_point();
    const auto knights_entry = m_tables[side_to_move]
                                   .knights_table[indices.knights_index]
                                   .to_fixed_point();
    const auto material_entry = m_tables[side_to_move]
                                    .material_table[indices.material_index]
                                    .to_fixed_point();

    const auto correction = (pawns_entry * CORR_HIST_PAWNS_WEIGHT)
                          + (diagonals_entry * CORR_HIST_DIAGONALS_WEIGHT)
                          + (orthogonals_entry * CORR_HIST_ORTHOGONALS_WEIGHT)
...
template <std::size_t size>
Correction_History_Indices Correction_History_Tables<size>::get_lemire_indices(
    const Zobrist_Hash& hash) const
{
    const Correction_History_Hashes hashes = hash.get_corr_hist_hashes();

    return {.pawns_index       = get_lemire_index(hashes.pawns_key),
