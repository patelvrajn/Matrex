...
constexpr Bitboard mask_pawn_attacks(PIECE_COLOR c, const Square& s)
{
    ...
    if (c == PIECE_COLOR::WHITE)
    {
        ...
        // forward-right. Moving diagonally forward-right is equivalent to
        // shifting the pawn's bit 7 positions right (since board is 8 wide,
        // right shift simulates moving north-east).
        const Bitboard RIGHT_ATTACK = (pawn_bb >> 7);

        // Moving diagonally forward-left is equivalent to shifting the pawn's
        // bit 9 positions right (north-west direction).
...
constexpr Bitboard mask_king_attacks(const Square& s)
{
    ...
    if (N_ATTACK.get_board()) { attacks |= N_ATTACK; }
    ...
    if (S_ATTACK.get_board()) { attacks |= S_ATTACK; }
