...
class Fixed_Point_Integer {
 public:
  ...
  constexpr Fixed_Point_Integer operator*(
      const Fixed_Point_Int_Storage_Type other) const;
  constexpr Fixed_Point_Integer operator+=(
      const Fixed_Point_Int_Storage_Type other);
  constexpr Fixed_Point_Integer operator-=(
      const Fixed_Point_Int_Storage_Type other);
  constexpr Fixed_Point_Integer operator/=(
      const Fixed_Point_Int_Storage_Type other);
  ...
  static constexpr double scale() { return static_cast<double>(1 << F); }

  static constexpr double precision() { return (1.0 / scale()); }
  ...
  static constexpr Fixed_Point_Integer from_integer(
      Fixed_Point_Int_Storage_Type integer) {
  ...
  static constexpr Fixed_Point_Int_Storage_Type lookup_log2_table(
      std::size_t index) {
    return m_log2_table[index];
  }

  static constexpr Fixed_Point_Int_Storage_Type lookup_exp2_table(
      std::size_t index) {
...
FORCE_INLINE constexpr uint8_t bit_width(Fixed_Point_Int_Storage_Type value) {
  if (value == 0) {
    ...
    // representable in Fixed_Point_Int_Storage_Type.
...
template <uint8_t F>
constexpr Fixed_Point_Integer<F> Fixed_Point_Integer<F>::operator+=(
    const Fixed_Point_Integer<F> other) {
...
Fixed_Point_Integer<F> log2(const Fixed_Point_Integer<F> input) {
  ...
  if (e >= 0) {
    MATREX_ASSERT(e < FIXED_POINT_BIT_WIDTH,
                  "LOG2 SHIFT ERROR: e is not in bounds, e is {}", e);
...
Fixed_Point_Integer<F> exp2(const Fixed_Point_Integer<F> input) {
  ...
  const Fixed_Point_Integer<F> i =
      (fractional_part *
       Fixed_Point_Integer<F>::from_integer(
           static_cast<Fixed_Point_Int_Storage_Type>(EXP2_LOOKUP_TABLE_SIZE)))
          .floor();
...
template <uint8_t F>
constexpr Fixed_Point_Integer<F> operator+(
    const Fixed_Point_Int_Storage_Type other, const Fixed_Point_Integer<F> fp) {
