...
constexpr Zobrist_Hash::Zobrist_Hash() : m_hash_value(0), m_corr_hist_hashes(Correction_History_Hashes()) {}
...
constexpr const Correction_History_Hashes& Zobrist_Hash::get_corr_hist_hashes() const
{
                                          ...
                                          const Square      square)
{
    ...
    {
        m_corr_hist_hashes.pawns_key ^= m_hash_keys.pieces[color][piece][square.get_index()];
    }

    if ((piece == PIECES::BISHOP) || (piece == PIECES::QUEEN))
    {
        m_corr_hist_hashes.diagonals_key ^= m_hash_keys.pieces[color][piece][square.get_index()];
    }
    
    if ((piece == PIECES::ROOK) || (piece == PIECES::QUEEN))
    {
        m_corr_hist_hashes.orthogonals_key ^= m_hash_keys.pieces[color][piece][square.get_index()];
    }
