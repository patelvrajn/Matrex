...
Bishop_Magic_Bitboards::Bishop_Magic_Bitboards()
{
    ...
    if (!m_is_attack_tables_initialized) { init_attack_hash_tables(); }
}
...
void Bishop_Magic_Bitboards::init_attack_hash_tables()
{
         ...
         square_idx++)
    {
        ...
        // moves.
        // Note: This saves memory compared to a fixed size array [64][512]
        // which assumes 512 different configurations per square.
        ...
        Magic_Hash_Table table(m_magics[square_idx],
                               num_of_high_bits_in_mask,
                               mask,
                               attacks);

        // Save the constructed hash table into the bishop’s global attack
        // tables, indexed by square. After this step, looking up bishop moves
        // for this square during gameplay will be O(1).
...
Bitboard
Bishop_Magic_Bitboards::calculate_bishop_attacks(const Square&   s,
