...
Chess_Board::Chess_Board(const Bitboard& w_pawn_bb,
                         const Bitboard& w_knight_bb,
                         const Bitboard& w_bishop_bb,
                         const Bitboard& w_rook_bb,
                         const Bitboard& w_queen_bb,
                         const Bitboard& w_king_bb,
                         const Bitboard& b_pawn_bb,
                         const Bitboard& b_knight_bb,
                         const Bitboard& b_bishop_bb,
                         const Bitboard& b_rook_bb,
...
void Chess_Board::pretty_print() const
{
    ...
    if (m_state.side_to_move == PIECE_COLOR::WHITE) { std::cout << "White"; }
    if (m_state.side_to_move == PIECE_COLOR::BLACK) { std::cout << "Black"; }
    if (m_state.side_to_move == PIECE_COLOR::NO_COLOR) { std::cout << "NONE"; }
    std::cout << std::endl;

    std::cout << "Enpassant Square: "
              << SQUARE_STRINGS[m_state.enpassant_square] << std::endl;

    std::cout << "Castling Rights: ";
    if (m_state.castling_rights == 0) { std::cout << "NONE"; }
    else
    {
...
Undo_Chess_Move Chess_Board::make_move(const Chess_Move& move)
{
    ...
    // If white moves their queenside rook or it is captured by black, white
    ...
    // If white moves their kingside rook or it is captured by black, white
    ...
    // If black moves their queenside rook or it is captured by white, black
    // loses queenside castling rights.
    if (((m_state.side_to_move == PIECE_COLOR::BLACK)
         && (move.moving_piece == PIECES::ROOK)
    ...
    // If black moves their kingside rook or it is captured by white, black
    // loses kingside castling rights.
    if (((m_state.side_to_move == PIECE_COLOR::BLACK)
         && (move.moving_piece == PIECES::ROOK)
