...
void epd_perft_test(std::string filepath)
{
    ...
    while (std::getline(iss, line))
    {
        ...
        while (start_of_perft_string != std::string::npos)
        {
            ...
            const std::string perft_string = line.substr(
                (start_of_perft_string + 1),
                ((next_semicolon - (start_of_perft_string + 1)) - 1));
            const size_t space_delimiter_position =
                perft_string.find_first_of(" ");
            const std::string depth_string =
                perft_string.substr(1, (space_delimiter_position - 1));
            const std::string leaf_node_count_string =
                perft_string.substr((space_delimiter_position + 1),
                                    ((perft_string.length() - 1)
                                     - (space_delimiter_position + 1) + 1));

            std::cout << "Perft depth: " << depth_string << std::endl;
            std::cout << "Expected perft count: " << leaf_node_count_string
                      << std::endl;

            const uint64_t perft_depth = std::stoull(depth_string);
            const uint64_t expected_perft_count =
            ...
            std::cout << "Perft(" << perft_depth << "): " << perft_count
                      << std::endl;
